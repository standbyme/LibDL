...
public interface MathMatrix extends MathIterator<MatrixScalar> {
    ...
    default MathMatrix transpose(){
        if (this instanceof Nd4jMatrix) {
            INDArray a = Nd4jMatrix.class.cast(this).getArray();
            return new Nd4jMatrix(a.transpose());
        } else{
            throw new UnsupportedOperationException();
        }
    }

	default MathMatrix dotProduct(MathMatrix matrix, boolean transpose){
        if (matrix instanceof Nd4jMatrix) {
            INDArray a = Nd4jMatrix.class.cast(this).getArray();
            INDArray b = Nd4jMatrix.class.cast(matrix).getArray();
            return new Nd4jMatrix(a.mul(b));
        } else{
            throw new UnsupportedOperationException();
        }
    }

    default MathMatrix multiply(Number num){
        if (this instanceof Nd4jMatrix) {
            INDArray a = Nd4jMatrix.class.cast(this).getArray();
            return new Nd4jMatrix(a.mul(num));
        } else{
            throw new UnsupportedOperationException();
        }
    }

    default MathMatrix dotProduct(MathMatrix matrix){
        return this.dotProduct(matrix,false);
    }
	/**
	 * 矩阵点积运算
	 * 
