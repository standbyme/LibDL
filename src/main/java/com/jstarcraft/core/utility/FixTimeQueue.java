 ...
 * @param <E>
 */
public class FixTimeQueue<E extends Delayed> extends AbstractQueue<E> implements BlockingQueue<E> {

	private class FixTimeIterator implements Iterator<E> {

		private final Object[] elements;
		private int cursor;
		private Object current = null;

		FixTimeIterator(Object[] elements) {
		...
		try {
			...
			for (;;) {
				...
				if (current == null) {
					...
					if (delay > 0) {
						boolean interrupt = condition.await(delay < fix ? delay : fix, TimeUnit.MILLISECONDS);
						if (!interrupt) {
							// 线程等待异常
							continue;
						}
			...
			for (;;) {
				...
				if (current == null) {
					if (expire <= 0)
						...
						boolean interrupt = condition.await(fix > expire ? expire : fix, TimeUnit.MILLISECONDS);
	...
	public Iterator<E> iterator() {
		return new FixTimeIterator(toArray());
	}
