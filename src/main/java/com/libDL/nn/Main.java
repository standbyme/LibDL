...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        inputLayer.setHasBias(true);
        outputLayer.setHasBias(true);

        inputLayer.setActivationFunction(new ActivationSigmoid());
        inputLayer.setLearnRate(0.01);
        INDArray w1 = Nd4j.randn(new int[]{4, 2}).muli(FastMath.sqrt(2.0 / (2 + 3)));
        INDArray w2 = Nd4j.randn(new int[]{1, 4}).muli(FastMath.sqrt(2.0 / (3 + 1)));


//        w1 = Nd4j.randn(new int[]{4, 2}, 2);
//        w2 = Nd4j.randn(new int[]{1, 4}, 12);
        inputLayer.setWeight(w1);
        inputLayer.setBias(Nd4j.zeros(4, 1));
        ...
        do {
            ...
            error = new LossNegativeLogLikelihood().computeGradient(Nd4j.create(XOR_IDEAL[i % 4]), outputLayer.getPreOutput(), new ActivationSoftmax(), null);
            double score = new LossNegativeLogLikelihood().computeScore(Nd4j.create(XOR_IDEAL[i % 4]), outputLayer.getPreOutput(), new ActivationSoftmax(), null, true);
