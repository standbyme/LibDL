...
public class Padding extends OperatorTensor {
    ...
    public Padding (Tensor input, int[] kernel_size, int[] padding, int[] stride, int[] dilation, boolean ceil_mode) {
        ...
        Supplier<INDArray> forward = () -> {
            ...
            long input_h = shape[rank - 2];
            long input_w = shape[rank - 1];

            long ceil_h = 0;
            long ceil_w = 0;
            if(ceil_mode) {
                int _filter_h = (kernel_size[0] - 1) * dilation[0] + 1;
                int _filter_w = (kernel_size[0] - 1) * dilation[1] + 1;
                long overage;
            ...
            if(padding[0] > 0 || padding[1] > 0 || ceil_h > 0 || ceil_w > 0) {
                shape[rank - 2] = input_h + padding[0] * 2 + ceil_h;
                shape[rank - 1] = input_w + padding[1] * 2 + ceil_w;
                INDArray inf = Nd4j.zeros(shape).assign(INF);
                INDArrayIndex[] indArrayIndices;
                if(rank == 3) {
                    indArrayIndices = new INDArrayIndex[] {
                            NDArrayIndex.all(),
                            NDArrayIndex.interval(padding[0], padding[0] + input.data.shape()[1]),
                            NDArrayIndex.interval(padding[1], padding[1] + input.data.shape()[2])
                    };
                }else {
                    indArrayIndices = new INDArrayIndex[] {
